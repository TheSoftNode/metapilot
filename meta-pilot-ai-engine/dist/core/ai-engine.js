...
class AIEngine extends events_1.EventEmitter {
    constructor(config) {
        ...
        this.performanceMonitor = new performance_1.PerformanceMonitor(this.logger);
        this.rateLimiter = new rate_limiter_1.RateLimiter(config.rateLimiting);
    }
    // ==================== INITIALIZATION ====================
    async initialize() {
        try {
    ...
    async loadCorePlugins() {
        ...
        try {
            ...
            const { NLPAnalyzer } = await Promise.resolve().then(() => __importStar(require('../plugins/nlp-analyzer')));
            const { ProposalAnalyzer } = await Promise.resolve().then(() => __importStar(require('../plugins/proposal-analyzer')));
    ...
    async analyze(type, input, context = {}, options = { priority: 'medium' }) {
        ...
        // Validate request
        try {
            core_1.AnalysisRequestSchema.parse(request);
        }
        catch (error) {
    ...
    async loadPlugin(pluginName, plugin) {
        try {
            ...
            if (plugin) {
                ...
                const pluginModule = await Promise.resolve(`${`../plugins/${pluginName}`}`).then(s => __importStar(require(s)));
