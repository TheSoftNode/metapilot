...
exports.ProposalAnalyzer = void 0;
const core_1 = require("../types/core");
class ProposalAnalyzer {
    constructor() {
        this.name = 'proposal-analyzer';
        this.version = '1.0.0';
        this.supportedTypes = ['proposal', 'dao', 'governance'];
        this.supportedBlockchains = ['ethereum', 'solana', 'polygon', 'arbitrum'];
        this.metadata = {
    ...
    async analyze(request) {
        ...
        try {
            if (!(0, core_1.isProposalAnalysis)(request.input)) {
                throw new Error('Invalid proposal input format');
            }
    ...
    async analyzeProposal(proposalInput, request) {
        const text = proposalInput.proposalText;
    ...
    assessRisks(text, proposalType) {
        ...
        const typePattern = this.daoPatterns[proposalType];
        if (typePattern) {
    ...
    analyzeBenefits(text, proposalType) {
        const textLower = text.toLowerCase();
        const benefits = [];
        let benefitScore = 0;
    ...
    analyzeStakeholderImpact(text) {
        ...
        const totalImpact = Object.values(stakeholders).reduce((sum, impact) => sum + impact.score, 0);
    ...
    assessStakeholderImpact(text, keywords) {
        const textLower = text.toLowerCase();
        let mentions = 0;
    ...
    assessImplementationFeasibility(text) {
        const textLower = text.toLowerCase();
    ...
    getHistoricalContext(daoName, proposalType) {
        ...
        const daoData = knownDAOs[daoKey] ||
    ...
    getRecommendedApproach(daoData, proposalType) {
    ...
    assessCommunityAlignment(text, context) {
        const textLower = text.toLowerCase();
    ...
    assessEconomicImpact(text, proposalInput) {
        const amounts = this.extractFinancialAmounts(text);
        const textLower = text.toLowerCase();
    ...
    generateDecision(analysis, proposalInput, context) {
    ...
    validate(request) {
        return (0, core_1.isProposalAnalysis)(request.input) &&
            !!(request.input.proposalText && request.input.proposalId);
    }
}
exports.ProposalAnalyzer = ProposalAnalyzer;
exports.default = ProposalAnalyzer;
//# sourceMappingURL=proposal-analyzer.js.map